...
function ResultScreen() {
  ...
  const chartData = [
    {
      label: 'Probability',
      data: results.chart_arr_PPM,
    },
  ];
  const chartData_PWM = [
    {
      label: 'Probability',
      data: results.chart_arr_PWM,
    },
  ];
  const axes = [
    ...
    <div className={styles.result}>
      ...
      <Card className={styles.resultContainer} elevation={Elevation.THREE}>
        <div className={styles.topResults}>
          <h1>Most likely bindings(PPM)</h1>
          {results.top_x_PPM.map((element) => (
      ...
      <Card className={styles.resultContainer} elevation={Elevation.THREE}>
        <div className={styles.topResults}>
          <h1>Most likely bindings(PWM)</h1>
          {results.top_x_PWM.map((element) => (
            <div key={element.id} className={styles.element}>
              <h4>{element.name}</h4>
              <p>Position: {element.position}</p>
              <p>Score: {element.value}</p>
            </div>
          ))}
        </div>
        <div className={styles.chart}>
